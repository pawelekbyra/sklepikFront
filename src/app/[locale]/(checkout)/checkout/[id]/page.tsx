...
async function CheckoutDataLoader({ params }: CheckoutPageProps) {
  ...
  const { id: cartId, locale } = await params;
  ...
  const [cartData, market, addressesData] = await Promise.all([
    ...
    resolveMarket(getDefaultCountry()).catch(() => null),
  ...
  if (cartData?.current_step === "complete") {
    redirect(`${buildBasePath(locale)}/order-placed/${cartId}`);
  }
  ...
  return <CheckoutPageContent cartId={cartId} initialData={initialData} />;
}
